...
export function AbraModule() {
  ...
  const closeNativeModal = () => {
    ...
    if (iframe && iframe.contentWindow) {
      iframe.contentWindow.postMessage({ type: 'CLOSE_IFRAME_REQUEST' }, '*')
    }
  }
  ...
  const handleDownloadRow = async (id: number, muestraCodigo?: string | null) => {
    ...
    try {
      const url = `${API_URL}/api/abra/excel?download=true&ensayo_id=${id}`
      ...
      let filename = `ABRA_${muestraCodigo || id}.xlsx`
