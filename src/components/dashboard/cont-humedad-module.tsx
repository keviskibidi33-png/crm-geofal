...
export function ContHumedadModule() {
  ...
  const fetchEnsayos = useCallback(async (): Promise<boolean> => {
    setLoading(true)
    try {
      const res = await authFetch(`${API_URL}/api/cont-humedad/?_ts=${Date.now()}`, {
        cache: "no-store",
      })
  ...
  const handleDeleteEnsayo = useCallback(
    async (id: number) => {
      ...
      if (!window.confirm(`Enviar a papelera el Ensayo de Contenido de Humedad ${ensayoLabel}? Se puede recuperar luego.`)) return
