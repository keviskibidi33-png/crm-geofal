...
import { Plus, Layers, Loader2, RefreshCw, Search, Eye, Pencil, Trash2, Download } from "lucide-react"
...
import PlanasForm from "./planas-native/PlanasForm"
...
export function PlanasModule() {
  ...
  const handleDownloadRow = async (id: number, muestraCodigo?: string | null) => {
    ...
    try {
      const url = `${API_URL}/api/planas/excel?download=true&ensayo_id=${id}`
      ...
      let filename = `PLANAS_${muestraCodigo || id}.xlsx`
    ...
    <div className="space-y-4 lg:space-y-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div className="min-w-0">
            <h2 className="text-xl sm:text-2xl font-bold tracking-tight leading-tight wrap-break-word">Partículas Planas y Alargadas ASTM D4791-19</h2>
            <p className="text-sm sm:text-base text-muted-foreground">Determinación de partículas planas y alargadas en agregados gruesos.</p>
          </div>
        </div>
      ...
      <div className="border rounded-xl shadow-sm bg-white">
        <div className="px-4 py-3 border-b bg-slate-50/70 rounded-t-xl">
          <h3 className="text-sm font-semibold text-slate-900">Historial Planas y Alargadas ASTM D4791-19</h3>
        ...
        <Table className="min-w-[860px]">
          ...
          <TableCaption className="text-xs text-muted-foreground">Planas y Alargadas ASTM D4791-19 - listado con busqueda y acceso rapido.</TableCaption>
        </Table>
        {!loading && filtered.length > 0 && (
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent className="max-w-[95vw] w-full h-[95vh] p-0 overflow-hidden bg-background [&>button]:hidden flex flex-col">
          <DialogHeader className="hidden">
            <DialogTitle>Ensayo Partículas Planas y Alargadas ASTM D4791-19</DialogTitle>
            <DialogDescription>Formulario Partículas Planas y Alargadas ASTM D4791-19</DialogDescription>
          </DialogHeader>
          <PlanasForm
      ...
      <Dialog open={isDetailOpen} onOpenChange={setIsDetailOpen}>
        <DialogContent className="max-w-xl">
          <DialogHeader>
            ...
            <DialogDescription>Informacion guardada del Ensayo Partículas Planas y Alargadas ASTM D4791-19.</DialogDescription>
          </DialogHeader>
          {selectedDetail ? (
            <div className="space-y-2 text-sm">
              ...
              <p><span className="font-semibold">Observaciones:</span> {selectedDetail.payload?.nota || "-"}</p>
            </div>
          ) : (
      ...
      <ModernConfirmDialog
        ...
        title="Eliminar Ensayo de Partículas Planas y Alargadas ASTM D4791-19"
