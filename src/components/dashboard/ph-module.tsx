...
import PHForm from "./ph-native/PHForm"
...
export function PHModule() {
  ...
  const fetchEnsayos = useCallback(async (): Promise<boolean> => {
    setLoading(true)
    try {
      const res = await authFetch(`${API_URL}/api/ph/?_ts=${Date.now()}`, {
        cache: "no-store",
      })
  ...
  const doOpenDetail = async (id: number) => {
    ...
    try {
      const res = await authFetch(`${API_URL}/api/ph/${id}?_ts=${Date.now()}`, { cache: "no-store" })
  ...
  const handleRefreshTable = useCallback(async () => {
    ...
    try {
      ...
      toast[ok ? "success" : "error"](
        ok ? "Tabla de pH NTP 339.176 actualizada." : "No se pudo actualizar la tabla de pH NTP 339.176.",
      )
    } finally {
  ...
  const handleDownloadRow = async (id: number, muestraCodigo?: string | null) => {
    ...
    try {
      const url = `${API_URL}/api/ph/excel?download=true&ensayo_id=${id}`
      ...
      let filename = `PH_${muestraCodigo || id}.xlsx`
    ...
    <div className="space-y-4 lg:space-y-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div className="min-w-0">
            <h2 className="text-xl sm:text-2xl font-bold tracking-tight leading-tight wrap-break-word">pH NTP 339.176</h2>
            <p className="text-sm sm:text-base text-muted-foreground">Determinación del valor pH en suelos y agua subterránea.</p>
          </div>
        </div>
      ...
      <div className="border rounded-xl shadow-sm bg-white">
        <div className="px-4 py-3 border-b bg-slate-50/70 rounded-t-xl">
          <h3 className="text-sm font-semibold text-slate-900">Historial pH NTP 339.176</h3>
        ...
        <Table className="min-w-[860px]">
          ...
          <TableCaption className="text-xs text-muted-foreground">pH NTP 339.176 - listado con busqueda y acceso rapido.</TableCaption>
        </Table>
        {!loading && filtered.length > 0 && (
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent className="max-w-[95vw] w-full h-[95vh] p-0 overflow-hidden bg-background [&>button]:hidden flex flex-col">
          <DialogHeader className="hidden">
            <DialogTitle>Ensayo pH NTP 339.176</DialogTitle>
            <DialogDescription>Formulario pH NTP 339.176</DialogDescription>
          </DialogHeader>
          <PHForm
      ...
      <Dialog open={isDetailOpen} onOpenChange={setIsDetailOpen}>
        <DialogContent className="max-w-xl">
          <DialogHeader>
            ...
            <DialogDescription>Informacion guardada del Ensayo pH NTP 339.176.</DialogDescription>
          </DialogHeader>
          {selectedDetail ? (
      ...
      <ModernConfirmDialog
        ...
        title="Eliminar Ensayo de pH NTP 339.176"
