...
import { Plus, Beaker, Loader2, AlertCircle, RefreshCw, Search, Eye, Pencil, Trash2, X, Download } from "lucide-react"
...
import SulfatosSolublesForm from "./sulfatos-solubles-native/SulfatosSolublesForm"
...
export function SulfatosSolublesModule() {
  ...
  const FRONTEND_URL = (
    process.env.NEXT_PUBLIC_SULFATOS_SOLUBLES_FRONTEND_URL ||
    process.env.NEXT_PUBLIC_SULFATOS_SOLUBLES_URL ||
  ...
  const fetchEnsayos = useCallback(async (): Promise<boolean> => {
    setLoading(true)
    try {
      const res = await authFetch(`${API_URL}/api/sulfatos-solubles/?_ts=${Date.now()}`, {
        cache: "no-store",
      })
      ...
      console.error("Error fetching Sulfatos Solubles ensayos", err)
      return false
    } finally {
      setLoading(false)
    }
  ...
  const openDetail = async (id: number) => {
    ...
    try {
      const res = await authFetch(`${API_URL}/api/sulfatos-solubles/${id}?_ts=${Date.now()}`, { cache: "no-store" })
  ...
  const handleRefreshTable = useCallback(async () => {
    ...
    try {
      ...
      toast[ok ? "success" : "error"](
        ok ? "Tabla de Sulfatos Solubles actualizada." : "No se pudo actualizar la tabla de Sulfatos Solubles.",
      )
    } finally {
  ...
  const handleDownloadRow = async (id: number, muestraCodigo?: string | null) => {
    ...
    try {
      const url = `${API_URL}/api/sulfatos-solubles/excel?download=true&ensayo_id=${id}`
      ...
      let filename = `SULFATOS_${muestraCodigo || id}.xlsx`
  ...
  const handleDeleteEnsayo = useCallback(
    async () => {
      ...
      try {
        const res = await authFetch(`${API_URL}/api/sulfatos-solubles/${id}`, { method: "DELETE" })
        if (!res.ok) throw new Error("No se pudo enviar a papelera el ensayo.")
        setEnsayos((prev) => prev.filter((row) => row.id !== id))
        toast.success("Ensayo de Sulfatos Solubles enviado a papelera.")
    ...
    <div className="space-y-4 lg:space-y-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div className="min-w-0">
            <h2 className="text-xl sm:text-2xl font-bold tracking-tight leading-tight wrap-break-word">Sulfato Suelo</h2>
      ...
      <div className="border rounded-xl shadow-sm bg-white">
        <div className="px-4 py-3 border-b bg-slate-50/70 rounded-t-xl">
          <h3 className="text-sm font-semibold text-slate-900">Historial Sulfato Suelo</h3>
        ...
        <Table className="min-w-[860px]">
          ...
          <TableCaption className="text-xs text-muted-foreground">Sulfatos Solubles - listado con busqueda y acceso rapido.</TableCaption>
        </Table>
        {!loading && filtered.length > 0 && (
      ...
      <Dialog open={isDetailOpen} onOpenChange={setIsDetailOpen}>
        <DialogContent className="max-w-xl">
          <DialogHeader>
            ...
            <DialogDescription>Informacion guardada del Ensayo Sulfatos Solubles.</DialogDescription>
          </DialogHeader>
          {selectedDetail ? (
      ...
      <ModernConfirmDialog
        ...
        title="Eliminar Ensayo de Sulfatos Solubles"
