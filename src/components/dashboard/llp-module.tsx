...
interface LLPEnsayoDetail extends LLPEnsayoSummary {
    payload?: {
...
export function LLPModule() {
    ...
    const [selectedDetail, setSelectedDetail] = useState<LLPEnsayoDetail | null>(null)
    ...
    const FRONTEND_URL = (
        ...
        process.env.NEXT_PUBLIC_LLP_URL ||
    ...
    const fetchEnsayos = useCallback(async (): Promise<boolean> => {
        setLoading(true)
        try {
            const res = await authFetch(`${API_URL}/api/llp/?_ts=${Date.now()}`, {
                cache: "no-store",
            })
            if (!res.ok) {
                return false
            }
            const data: LLPEnsayoSummary[] = await res.json()
            setEnsayos(data)
            return true
        } catch (err) {
            console.error('Error fetching LLP ensayos', err)
            return false
        } finally {
            setLoading(false)
        }
    ...
    const openDetail = async (id: number) => {
        ...
        try {
            const res = await authFetch(`${API_URL}/api/llp/${id}?_ts=${Date.now()}`, {
                cache: "no-store",
            })
            if (!res.ok) {
                throw new Error("No se pudo cargar el detalle.")
            }
            const data: LLPEnsayoDetail = await res.json()
    ...
    const handleRefreshTable = useCallback(async () => {
        ...
        try {
            ...
            if (ok) {
                toast.success("Tabla de LLP actualizada.")
            } else {
                toast.error("No se pudo actualizar la Tabla de LLP.")
            }
        } finally {
    ...
    const handleDownloadRow = async (id: number, muestraCodigo?: string | null) => {
        ...
        try {
            const url = `${API_URL}/api/llp/excel?download=true&ensayo_id=${id}`
            ...
            let filename = `LLP_${muestraCodigo || id}.xlsx`
                            ...
                            <TableRow key={ensayo.id} className="hover:bg-slate-50">
                                ...
                                <TableCell className="text-right">
                                    <div className="flex items-center justify-end gap-2">
                                        ...
                                        <Button variant="ghost" size="icon" className="text-destructive hover:text-destructive hover:bg-destructive/10" onClick={() => confirmDelete(ensayo.id)} disabled={deletingEnsayoId === ensayo.id} title="Eliminar ensayo">
            ...
            <Dialog open={isDetailOpen} onOpenChange={setIsDetailOpen}>
                <DialogContent className="max-w-xl">
                    <DialogHeader>
                        ...
                        <DialogDescription>Información guardada del Ensayo LLP.</DialogDescription>
                    </DialogHeader>
                    {selectedDetail ? (
                        <div className="space-y-2 text-sm">
                            ...
                            <p><span className="font-semibold">LL promedio:</span> {selectedDetail.limite_liquido_promedio ?? "-"}</p>
                            <p><span className="font-semibold">LP promedio:</span> {selectedDetail.limite_plastico_promedio ?? "-"}</p>
                            <p><span className="font-semibold">Índice plasticidad:</span> {selectedDetail.indice_plasticidad ?? "-"}</p>
