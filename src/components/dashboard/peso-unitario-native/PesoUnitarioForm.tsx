    ...
    <div className="min-h-screen bg-slate-100 p-4 md:p-6 overflow-y-auto">
      <div className="mx-auto max-w-[1280px] space-y-4">
        ...
        <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
          ...
          <button onClick={() => void save(false)} disabled={loading} className="flex h-11 items-center justify-center gap-2 rounded-lg border border-slate-900 bg-white font-semibold text-slate-900 shadow-sm transition hover:bg-slate-100 disabled:opacity-50">{loading ? <><Loader2 className="h-4 w-4 animate-spin" />Guardando...</> : <><Save className="h-4 w-4" />Guardar</>}</button>
          <button onClick={() => void save(true)} disabled={loading} className="flex h-11 items-center justify-center gap-2 rounded-lg border border-emerald-700 bg-emerald-700 font-semibold text-white shadow-sm transition hover:bg-emerald-800 disabled:opacity-50">{loading ? <><Loader2 className="h-4 w-4 animate-spin" />Procesando...</> : <><Download className="h-4 w-4" />Guardar y Exportar</>}</button>
        </div>
      </div>
    </div>
  )
