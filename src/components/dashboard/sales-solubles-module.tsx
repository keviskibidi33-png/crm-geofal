...
import SalesSolublesForm from "./sales-solubles-native/SalesSolublesForm"
...
export function SalesSolublesModule() {
  ...
  const FRONTEND_URL = (
    process.env.NEXT_PUBLIC_SALES_SOLUBLES_FRONTEND_URL ||
    process.env.NEXT_PUBLIC_SALES_SOLUBLES_URL ||
  ...
  const fetchEnsayos = useCallback(async (): Promise<boolean> => {
    setLoading(true)
    try {
      const res = await authFetch(`${API_URL}/api/sales-solubles/?_ts=${Date.now()}`, {
        cache: "no-store",
      })
      ...
      console.error("Error fetching Sales Solubles ensayos", err)
      return false
    } finally {
      setLoading(false)
    }
  ...
  const openDetail = async (id: number) => {
    ...
    try {
      const res = await authFetch(`${API_URL}/api/sales-solubles/${id}?_ts=${Date.now()}`, { cache: "no-store" })
  ...
  const handleRefreshTable = useCallback(async () => {
    ...
    try {
      ...
      toast[ok ? "success" : "error"](
        ok ? "Tabla de Sales Solubles actualizada." : "No se pudo actualizar la tabla de Sales Solubles.",
      )
    } finally {
  ...
  const handleDownloadRow = async (id: number, muestraCodigo?: string | null) => {
    ...
    try {
      const url = `${API_URL}/api/sales-solubles/excel?download=true&ensayo_id=${id}`
      ...
      let filename = `SALES_${muestraCodigo || id}.xlsx`
  ...
  const handleDeleteEnsayo = useCallback(
    async () => {
      ...
      try {
        const res = await authFetch(`${API_URL}/api/sales-solubles/${id}`, { method: "DELETE" })
        if (!res.ok) throw new Error("No se pudo enviar a papelera el ensayo.")
        setEnsayos((prev) => prev.filter((row) => row.id !== id))
        toast.success("Ensayo de Sales Solubles enviado a papelera.")
  ...
  return (
    <div className="space-y-4 lg:space-y-6">
      ...
      <Dialog open={isDetailOpen} onOpenChange={setIsDetailOpen}>
        <DialogContent className="max-w-xl">
          <DialogHeader>
            ...
            <DialogDescription>Informacion guardada del Ensayo Sales Solubles.</DialogDescription>
          </DialogHeader>
          {selectedDetail ? (
      ...
      <ModernConfirmDialog
        ...
        title="Eliminar Ensayo de Sales Solubles"
