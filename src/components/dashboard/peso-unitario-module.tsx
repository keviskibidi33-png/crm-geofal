...
import { Plus, Scale, Loader2, RefreshCw, Search, Eye, Pencil, Trash2, Download } from "lucide-react"
...
import PesoUnitarioForm from "./peso-unitario-native/PesoUnitarioForm"
...
export function PesoUnitarioModule() {
  ...
  const fetchEnsayos = useCallback(async (): Promise<boolean> => {
    setLoading(true)
    try {
      const res = await authFetch(`${API_URL}/api/peso-unitario/?_ts=${Date.now()}`, {
        cache: "no-store",
      })
  ...
  const handleRefreshTable = useCallback(async () => {
    ...
    try {
      ...
      toast[ok ? "success" : "error"](
        ok ? "Tabla de Peso Unitario actualizada." : "No se pudo actualizar la tabla de Peso Unitario.",
      )
    } finally {
  ...
  const handleDownloadRow = async (id: number, muestraCodigo?: string | null) => {
    ...
    try {
      const url = `${API_URL}/api/peso-unitario/excel?download=true&ensayo_id=${id}`
      ...
      let filename = `PESO_UNITARIO_${muestraCodigo || id}.xlsx`
  ...
  const handleDeleteEnsayo = useCallback(
    async () => {
      ...
      try {
        const res = await authFetch(`${API_URL}/api/peso-unitario/${id}`, { method: "DELETE" })
        if (!res.ok) throw new Error("No se pudo enviar a papelera el ensayo.")
        setEnsayos((prev) => prev.filter((row) => row.id !== id))
        toast.success("Ensayo de Peso Unitario enviado a papelera.")
    ...
    <div className="space-y-4 lg:space-y-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="text-sm sm:text-base text-muted-foreground">Determinación de la densidad aparente (peso unitario) y vacíos en agregados.</p>
          </div>
        </div>
      ...
      <div className="border rounded-xl shadow-sm bg-white">
        <div className="px-4 py-3 border-b bg-slate-50/70 rounded-t-xl">
          <h3 className="text-sm font-semibold text-slate-900">Historial Peso Unitario ASTM C29/C29M-23</h3>
        ...
        <Table className="min-w-[860px]">
          ...
          <TableCaption className="text-xs text-muted-foreground">Peso Unitario ASTM C29/C29M-23 - listado con busqueda y acceso rapido.</TableCaption>
        </Table>
        {!loading && filtered.length > 0 && (
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent className="max-w-[95vw] w-full h-[95vh] p-0 overflow-hidden bg-background [&>button]:hidden flex flex-col">
          <DialogHeader className="hidden">
            <DialogTitle>Ensayo Peso Unitario ASTM C29/C29M-23</DialogTitle>
      ...
      <Dialog open={isDetailOpen} onOpenChange={setIsDetailOpen}>
        <DialogContent className="max-w-xl">
          <DialogHeader>
            ...
            <DialogDescription>Informacion guardada del Ensayo Peso Unitario ASTM C29/C29M-23.</DialogDescription>
          </DialogHeader>
          {selectedDetail ? (
      ...
      <ModernConfirmDialog
        ...
        title="Eliminar Ensayo de Peso Unitario ASTM C29/C29M-23"
