...
import { Plus, TestTube, Loader2, RefreshCw, Search, Eye, Pencil, Trash2, Download } from "lucide-react"
...
export function EquiArenaModule() {
  const config = useMemo(() => getModuleConfig("equi-arena"), [])
  ...
  const handleRefreshTable = useCallback(async () => {
    ...
    try {
      ...
      toast[ok ? "success" : "error"](
        ok ? "Tabla de Equivalente de Arena actualizada." : "No se pudo actualizar la tabla de Equivalente de Arena.",
      )
    } finally {
  ...
  const handleDownloadRow = async (id: number, muestraCodigo?: string | null) => {
    ...
    try {
      const url = `${API_URL}/api/equi-arena/excel?download=true&ensayo_id=${id}`
      ...
      let filename = `EQUI_ARENA_${muestraCodigo || id}.xlsx`
  ...
  const handleDeleteEnsayo = useCallback(
    async () => {
      ...
      try {
        ...
        toast.success("Ensayo de Equivalente de Arena enviado a papelera.")
    ...
    <div className="space-y-4 lg:space-y-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div className="min-w-0">
            <h2 className="text-xl sm:text-2xl font-bold tracking-tight leading-tight wrap-break-word">Equivalente de Arena</h2>
            <p className="text-sm sm:text-base text-muted-foreground">Determinacion del equivalente de arena en suelos y agregados.</p>
          </div>
        </div>
      ...
      <div className="border rounded-xl shadow-sm bg-white">
        <div className="px-4 py-3 border-b bg-slate-50/70 rounded-t-xl">
          <h3 className="text-sm font-semibold text-slate-900">Historial Equivalente de Arena</h3>
        ...
        <Table className="min-w-[860px]">
          ...
          <TableCaption className="text-xs text-muted-foreground">Equivalente de Arena - listado con busqueda y acceso rapido.</TableCaption>
        </Table>
        {!loading && filtered.length > 0 && (
      ...
      {isNative ? (
        <NativeEnsayoModals
          ...
          apiUrl={API_URL}
          iframeSrc={iframeSrc}
          iframeTitle="Equivalente de Arena CRM"
          ...
          <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
            <DialogContent className="max-w-[95vw] w-full h-[95vh] p-0 overflow-hidden bg-background [&>button]:hidden">
              <DialogHeader className="hidden">
                <DialogTitle>Ensayo Equivalente de Arena</DialogTitle>
                <DialogDescription>Formulario Equivalente de Arena</DialogDescription>
              </DialogHeader>
              <iframe src={iframeSrc} className="w-full h-full border-none" title="Equivalente de Arena CRM" />
            </DialogContent>
          </Dialog>
          ...
          <Dialog open={isDetailOpen} onOpenChange={setIsDetailOpen}>
            <DialogContent className="max-w-xl">
              <DialogHeader>
                ...
                <DialogDescription>Informacion guardada del Ensayo Equivalente de Arena.</DialogDescription>
              </DialogHeader>
              {selectedDetail ? (
      ...
      <ModernConfirmDialog
        ...
        title="Eliminar Ensayo de Equivalente de Arena"
