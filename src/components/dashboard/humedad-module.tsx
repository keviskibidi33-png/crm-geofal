...
import { Plus, Droplets, Loader2, AlertCircle, RefreshCw, Search, Eye, Pencil, Trash2, X, Download } from "lucide-react"
...
import HumedadForm from "./humedad-native/HumedadForm"
...
export function HumedadModule() {
    ...
    const fetchEnsayos = useCallback(async (): Promise<boolean> => {
        setLoading(true)
        try {
            const res = await authFetch(`${API_URL}/api/humedad/?_ts=${Date.now()}`, {
                cache: "no-store",
            })
            if (!res.ok) {
                return false
    ...
    const openDetail = async (id: number) => {
        ...
        try {
            const res = await authFetch(`${API_URL}/api/humedad/${id}?_ts=${Date.now()}`, {
                cache: "no-store",
            })
            if (!res.ok) {
    ...
    const handleDownloadRow = async (id: number, muestraCodigo?: string | null) => {
        ...
        try {
            const url = `${API_URL}/api/humedad/excel?download=true&ensayo_id=${id}`
            ...
            let filename = `HUMEDAD_${muestraCodigo || id}.xlsx`
    ...
    const handleDeleteEnsayo = useCallback(async () => {
        ...
        try {
            ...
            toast.success("Ensayo de humedad enviado a papelera.")
    ...
    const handleRefreshTable = useCallback(async () => {
        ...
        try {
            ...
            if (ok) {
                toast.success("Tabla de humedad actualizada.")
            } else {
    ...
    return (
        <div className="space-y-4 lg:space-y-6">
            ...
            <ModernConfirmDialog
                ...
                title="Eliminar Ensayo de Humedad"
