...
interface OficinaTecnicaModuleProps {
    user: User
}

const TOKEN_BRIDGE_TRACE_PREFIX = "[OficinaTecnicaTokenBridge]"
...
export function OficinaTecnicaModule({ user }: OficinaTecnicaModuleProps) {
    ...
    const iframeUrl = useMemo(() => {
        ...
        return resolveFrontendModuleUrl(
            process.env.NEXT_PUBLIC_PROGRAMACION_OFICINA_TECNICA_URL || process.env.NEXT_PUBLIC_PROGRAMACION_URL,
            fallbackUrl,
            "programacion-oficina-tecnica",
        )
    }, [])
        ...
        const handleMessage = (event: MessageEvent) => {
            ...
            if (event.data?.type === 'TOKEN_REFRESH_REQUEST' && event.source) {
                ...
                if (immediateToken) {
                    ;(event.source as Window).postMessage(
                        { type: 'TOKEN_REFRESH', token: immediateToken, requestId, source: 'oficina_tecnica_module_immediate' },
                        event.origin
                    )
                ...
                syncIframeToken(`request:${requestId || "none"}`).then((freshToken) => {
                    if (freshToken && event.source) {
                        (event.source as Window).postMessage(
                            { type: 'TOKEN_REFRESH', token: freshToken, requestId, source: 'oficina_tecnica_module_sync' },
                            event.origin
                        )
            ...
            if (event.data?.type === 'AUTH_REQUIRED') {
                ...
                syncIframeToken(`auth-required:${event.data?.requestId || "none"}`).then((freshToken) => {
                    if (freshToken && event.source) {
                        (event.source as Window).postMessage(
                            { type: 'TOKEN_REFRESH', token: freshToken, requestId: event.data?.requestId, source: 'oficina_tecnica_module_recovery' },
                            event.origin
                        )
                        return
                    }
    ...
    const openModule = useCallback(async () => {
        ...
        if (!token) {
            handleIframeSessionFailure("open:oficina_tecnica")
            return
        }
        ...
        <div className="flex flex-col h-full space-y-4 p-4 bg-zinc-50/50 overflow-y-auto">
            ...
            <div className="flex items-center justify-between border-b border-zinc-200 pb-4">
                <div>
                    ...
                    <p className="text-zinc-500 text-sm font-medium">Seguimiento técnico limitado para entrega real y estado.</p>
                </div>
            ...
            <div className="flex items-center justify-between bg-white px-6 py-3 rounded-xl border border-zinc-200 shadow-sm">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h4 className="text-sm font-bold text-zinc-900">Matriz de Oficina Técnica</h4>
                        <p className="text-[11px] text-zinc-500 font-medium">Acceso directo al seguimiento operativo restringido.</p>
                    </div>
                </div>
                <Button
                    onClick={openModule}
                ...
                <CardContent className="p-0">
                    {recentChanges.length > 0 ? (
                        ...
                        <div className="p-4 text-center text-xs text-zinc-400 font-medium italic">
                            Sin cambios técnicos recientes detectados.
                        </div>
                    )}
                </CardContent>
            </Card>
            ...
            {/* Modal de Tabla Oficina Tecnica (Fullscreen) */}
                    ...
                    <div className="flex-none flex items-center justify-between px-6 py-2 bg-white border-b border-zinc-200">
                        <div className="flex items-center gap-3">
                            ...
                            <DialogTitle className="font-bold text-zinc-900 text-xs uppercase tracking-widest">
                                Control Oficina Técnica - Matriz de Datos
                            </DialogTitle>
                            <DialogDescription className="sr-only">
                                Módulo de oficina técnica de programación para entrega real y estado de trabajo.
                            </DialogDescription>
                        </div>
                        <div className="flex items-center gap-3">
