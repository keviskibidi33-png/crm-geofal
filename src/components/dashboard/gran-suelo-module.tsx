...
import { Plus, Filter, Loader2, RefreshCw, Search, Eye, Pencil, Trash2, Download, FlaskConical } from "lucide-react"
...
export function GranSueloModule() {
  const config = useMemo(() => getModuleConfig("gran-suelo"), [])
  ...
  const handleRefreshTable = useCallback(async () => {
    ...
    try {
      ...
      toast[ok ? "success" : "error"](
        ok ? "Tabla de GRANULOMETRIA SUELOS ASTM D6913/D6913M-17 actualizada." : "No se pudo actualizar la tabla de GRANULOMETRIA SUELOS ASTM D6913/D6913M-17.",
      )
    } finally {
  ...
  const handleDownloadRow = async (id: number, muestraCodigo?: string | null) => {
    ...
    try {
      const url = `${API_URL}/api/gran-suelo/excel?download=true&ensayo_id=${id}`
      ...
      let filename = `GRAN_SUELO_${muestraCodigo || id}.xlsx`
    ...
    <div className="space-y-4 lg:space-y-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="text-sm sm:text-base text-muted-foreground">Analisis granulometrico de suelos por tamizado.</p>
          </div>
        </div>
      ...
      <div className="border rounded-xl shadow-sm bg-white">
        <div className="px-4 py-3 border-b bg-slate-50/70 rounded-t-xl">
          <h3 className="text-sm font-semibold text-slate-900">Historial GRANULOMETRIA SUELOS ASTM D6913/D6913M-17</h3>
        ...
        <Table className="min-w-[860px]">
          ...
          <TableCaption className="text-xs text-muted-foreground">GRANULOMETRIA SUELOS ASTM D6913/D6913M-17 - listado con busqueda y acceso rapido.</TableCaption>
        </Table>
        {!loading && filtered.length > 0 && (
          ...
          <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
            <DialogContent className="max-w-[95vw] w-full h-[95vh] p-0 overflow-hidden bg-background [&>button]:hidden">
              <DialogHeader className="hidden">
                <DialogTitle>Ensayo GRANULOMETRIA SUELOS ASTM D6913/D6913M-17</DialogTitle>
                <DialogDescription>Formulario GRANULOMETRIA SUELOS ASTM D6913/D6913M-17</DialogDescription>
              </DialogHeader>
              <iframe src={iframeSrc} className="w-full h-full border-none" title="GRANULOMETRIA SUELOS ASTM D6913/D6913M-17 CRM" />
            </DialogContent>
          </Dialog>
          ...
          <Dialog open={isDetailOpen} onOpenChange={setIsDetailOpen}>
            <DialogContent className="max-w-xl">
              <DialogHeader>
                ...
                <DialogDescription>Informacion guardada del Ensayo GRANULOMETRIA SUELOS ASTM D6913/D6913M-17.</DialogDescription>
              </DialogHeader>
              {selectedDetail ? (
      ...
      <ModernConfirmDialog
        ...
        title="Eliminar Ensayo de GRANULOMETRIA SUELOS ASTM D6913/D6913M-17"
