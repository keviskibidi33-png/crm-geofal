...
function VincularMuestrasModal({ isOpen, onClose, onSave, apiUrl }: VincularMuestrasModalProps) {
  ...
  useEffect(() => {
    ...
    const searchReceptions = async () => {
      try {
        setLoading(true)
        const res = await authFetch(`${apiUrl}/api/recepcion/paginated?q=${encodeURIComponent(debouncedSearch)}&page_size=10`)
      ...
      <DialogContent className="max-w-4xl max-h-[85vh] flex flex-col p-6 overflow-hidden bg-white dark:bg-zinc-950">
        ...
        <div className="flex-1 overflow-y-auto my-4 pr-1 space-y-4 overflow-visible">
          ...
          <div className="space-y-1.5 relative overflow-visible">
            <label className="text-xs font-bold text-slate-650 dark:text-zinc-400">
              Buscar Recepción en el Sistema
            </label>
            <div className="relative">
              ...
              <Input
                ...
                placeholder="Ingresa OT, Recepción o nombre del cliente..."
            ...
            {/* Suggestions list */}
            {receptions.length > 0 && (
              <div className="absolute left-0 right-0 top-full mt-1 bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-lg shadow-lg z-[100] max-h-60 overflow-y-auto divide-y divide-slate-100 dark:divide-zinc-800">
                {receptions.map((rec) => (
                  <button
                    key={rec.id}
                    type="button"
                    onClick={() => handleSelectReception(rec)}
                    className="w-full text-left px-4 py-2.5 text-xs hover:bg-slate-50 dark:hover:bg-zinc-850 flex flex-col gap-0.5 transition-colors"
                  >
                    <div className="flex justify-between font-bold text-slate-900 dark:text-zinc-200">
                      <span>OT: {rec.numero_ot}</span>
                      <span className="text-slate-500 font-mono text-[10px]">REC: {rec.numero_recepcion}</span>
                    </div>
                    <div className="text-[10px] text-slate-500 dark:text-zinc-400 truncate">
                      Cliente: {rec.cliente} | Proyecto: {rec.proyecto}
                    </div>
                  </button>
                ))}
              </div>
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                <h4 className="text-sm font-bold text-slate-800 dark:text-zinc-100 flex items-center gap-1.5">
                  ...
                  Muestras Registradas ({samples.length})
                </h4>
                <Button
                  type="button"
                  size="sm"
