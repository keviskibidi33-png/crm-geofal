...
export function CarasModule() {
  const config = useMemo(() => getModuleConfig("caras"), [])
  ...
  const FRONTEND_URL = (
    process.env.NEXT_PUBLIC_CARAS_FRONTEND_URL ||
    process.env.NEXT_PUBLIC_CARAS_URL ||
  ...
  const handleRefreshTable = useCallback(async () => {
    ...
    try {
      ...
      toast[ok ? "success" : "error"](
        ok ? "Tabla de CARAS ASTM D5821-13 actualizada." : "No se pudo actualizar la tabla de CARAS ASTM D5821-13.",
      )
    } finally {
  ...
  const handleDownloadRow = async (id: number, muestraCodigo?: string | null) => {
    ...
    try {
      ...
      let filename = `CARAS_${muestraCodigo || id}.xlsx`
  ...
  const handleDeleteEnsayo = useCallback(
    async () => {
      ...
      try {
        ...
        toast.success("Ensayo de CARAS ASTM D5821-13 enviado a papelera.")
    ...
    <div className="space-y-4 lg:space-y-6">
      ...
      <div className="border rounded-xl shadow-sm bg-white">
        <div className="px-4 py-3 border-b bg-slate-50/70 rounded-t-xl">
          <h3 className="text-sm font-semibold text-slate-900">Historial CARAS ASTM D5821-13</h3>
        ...
        <Table className="min-w-[860px]">
          ...
          <TableCaption className="text-xs text-muted-foreground">CARAS ASTM D5821-13 - listado con busqueda y acceso rapido.</TableCaption>
        </Table>
        {!loading && filtered.length > 0 && (
      ...
      {isNative ? (
        <NativeEnsayoModals
          ...
          apiUrl={API_URL}
          iframeSrc={iframeSrc}
          iframeTitle="Caras Fracturadas CRM"
          ...
          <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
            <DialogContent className="max-w-[95vw] w-full h-[95vh] p-0 overflow-hidden bg-background [&>button]:hidden">
              <DialogHeader className="hidden">
                <DialogTitle>Ensayo CARAS ASTM D5821-13</DialogTitle>
                <DialogDescription>Formulario CARAS ASTM D5821-13</DialogDescription>
              </DialogHeader>
              <iframe src={iframeSrc} className="w-full h-full border-none" title="CARAS ASTM D5821-13 CRM" />
            </DialogContent>
          </Dialog>
          ...
          <Dialog open={isDetailOpen} onOpenChange={setIsDetailOpen}>
            <DialogContent className="max-w-xl">
              <DialogHeader>
                ...
                <DialogDescription>Informacion guardada del Ensayo CARAS ASTM D5821-13.</DialogDescription>
              </DialogHeader>
              {selectedDetail ? (
      ...
      <ModernConfirmDialog
        ...
        title="Eliminar Ensayo de CARAS ASTM D5821-13"
