...
import CloroSolubleForm from "./cloro-soluble-native/CloroSolubleForm"
...
export function CloroSolubleModule() {
  ...
  const FRONTEND_URL = (
    process.env.NEXT_PUBLIC_CLORO_SOLUBLE_FRONTEND_URL ||
    process.env.NEXT_PUBLIC_CLORO_SOLUBLE_URL ||
  ...
  const fetchEnsayos = useCallback(async (): Promise<boolean> => {
    setLoading(true)
    try {
      const res = await authFetch(`${API_URL}/api/cloro-soluble/?_ts=${Date.now()}`, {
        cache: "no-store",
      })
      ...
      console.error("Error fetching Cloro Soluble ensayos", err)
      return false
    } finally {
      setLoading(false)
    }
  ...
  const openDetail = async (id: number) => {
    ...
    try {
      const res = await authFetch(`${API_URL}/api/cloro-soluble/${id}?_ts=${Date.now()}`, { cache: "no-store" })
  ...
  const handleRefreshTable = useCallback(async () => {
    ...
    try {
      ...
      toast[ok ? "success" : "error"](
        ok ? "Tabla de Cloro Soluble actualizada." : "No se pudo actualizar la tabla de Cloro Soluble.",
      )
    } finally {
  ...
  const handleDownloadRow = async (id: number, muestraCodigo?: string | null) => {
    ...
    try {
      const url = `${API_URL}/api/cloro-soluble/excel?download=true&ensayo_id=${id}`
      ...
      let filename = `CLORO_${muestraCodigo || id}.xlsx`
  ...
  const handleDeleteEnsayo = useCallback(
    async () => {
      ...
      try {
        const res = await authFetch(`${API_URL}/api/cloro-soluble/${id}`, { method: "DELETE" })
        if (!res.ok) throw new Error("No se pudo enviar a papelera el ensayo.")
        setEnsayos((prev) => prev.filter((row) => row.id !== id))
        toast.success("Ensayo de Cloro Soluble enviado a papelera.")
  ...
  return (
    <div className="space-y-4 lg:space-y-6">
      ...
      <Dialog open={isDetailOpen} onOpenChange={setIsDetailOpen}>
        <DialogContent className="max-w-xl">
          <DialogHeader>
            ...
            <DialogDescription>Informacion guardada del Ensayo Cloro Soluble.</DialogDescription>
          </DialogHeader>
          {selectedDetail ? (
      ...
      <ModernConfirmDialog
        ...
        title="Eliminar Ensayo de Cloro Soluble"
