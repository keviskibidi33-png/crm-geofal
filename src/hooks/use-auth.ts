...
export type ModuleType = "clientes" | "cotizadora" | "configuracion" | "proyectos" | "usuarios" | "auditoria" | "programacion" | "permisos" | "laboratorio" | "comercial" | "administracion" | "verificacion_muestras" | "recepcion" | "compresion" | "tracing" | "humedad" | "cbr" | "proctor" | "llp" | "gran_suelo" | "gran_agregado" | "abra" | "peso_unitario" | "equi_arena" | "ge_fino" | "ge_grueso"
...
async function buildUser(session: any): Promise<User> {
    ...
    const enforcePermissions = (perms: RolePermissions): RolePermissions => {
        ...
        if (!p.ge_fino) {
            p.ge_fino = {
                read: p.equi_arena?.read || p.peso_unitario?.read || p.abra?.read || p.gran_agregado?.read || p.llp?.read || p.proctor?.read || false,
                write: p.equi_arena?.write || p.peso_unitario?.write || p.abra?.write || p.gran_agregado?.write || p.llp?.write || p.proctor?.write || false,
                delete: p.equi_arena?.delete || p.peso_unitario?.delete || p.abra?.delete || p.gran_agregado?.delete || p.llp?.delete || p.proctor?.delete || false,
            }
        }
        if (!p.ge_grueso) {
            p.ge_grueso = {
                read: p.ge_fino?.read || p.equi_arena?.read || p.peso_unitario?.read || p.abra?.read || p.gran_agregado?.read || p.llp?.read || p.proctor?.read || false,
                write: p.ge_fino?.write || p.equi_arena?.write || p.peso_unitario?.write || p.abra?.write || p.gran_agregado?.write || p.llp?.write || p.proctor?.write || false,
                delete: p.ge_fino?.delete || p.equi_arena?.delete || p.peso_unitario?.delete || p.abra?.delete || p.gran_agregado?.delete || p.llp?.delete || p.proctor?.delete || false,
            }
        }
