  ...
  const fetchKpis = useCallback(async () => {
    try {
      ...
      const [pfRes, ppRes, peRes, eEntRes, eProRes, eInfRes, eAnuRes, tATRes, tCRRes, evRecRes, evInfRes, sTotalRes, sEmsRes, sDenRes, sProbRes, pfHoyRes, pfAyerRes, pfRestoRes, stEntRes, stInfRes, stNoIndRes] = await Promise.all([
        ...
        supabase.from("programacion_lab").select("id", { count: "exact", head: true }).eq("estado_trabajo", "ENTREGADO").gte("created_at", startDate).lt("created_at", endDate),
        supabase.from("programacion_lab").select("id", { count: "exact", head: true }).eq("estado_trabajo", "PROCESO").gte("created_at", startDate).lt("created_at", endDate),
        supabase.from("programacion_lab").select("id", { count: "exact", head: true }).eq("estado_trabajo", "INFORME LISTO").gte("created_at", startDate).lt("created_at", endDate),
        supabase.from("programacion_lab").select("id", { count: "exact", head: true }).eq("estado_trabajo", "ANULADO").gte("created_at", startDate).lt("created_at", endDate),
        supabase.from("programacion_lab").select("id", { count: "exact", head: true }).not("entrega_real", "is", null).not("fecha_entrega_estimada", "is", null).lte("entrega_real", "fecha_entrega_estimada").gte("created_at", startDate).lt("created_at", endDate),
